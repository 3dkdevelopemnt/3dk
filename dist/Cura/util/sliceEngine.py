...
class EngineResult(object):
	...
	def submitInfoOnline(self):
		...
		data = {
			...
			'simpleModeProfile': profile.getPreference('simpleModeProfile'),
			'simpleModeMaterial': profile.getPreference('simpleModeMaterial'),
		}
		try:
			f = urllib2.urlopen("http://www.fracktal.in/FracktoryStats/", data = urllib.urlencode(data), timeout = 3)
			f.read()
			f.close()
		except:
			import traceback
