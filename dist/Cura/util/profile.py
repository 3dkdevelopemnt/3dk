...
setting('start.gcode', """;Sliced for {machine_name} - {nozzle_size} nozzle
...
""", str, 'alteration', 'alteration')
#######################################################################################
setting('start2.gcode', """;Sliced for {machine_name} - {nozzle_size} nozzle
;Temperatures: {temperature_info}
;Sliced at {day} {date} {time}
;Basic settings: Layer height: {layer_height} Walls: {wall_thickness} Fill: {fill_density}
;Print time: {print_time}
...
setting('machine_variant', 'UNK', str, 'machine', 'hidden')
...
def replaceTagMatch(m):
	...
	if tag == 'nozzle_size':
		return pre + str(getProfileSettingFloat('nozzle_size'))
	if tag == 'temperature_info':
		res = "Nozzle"  
		if int(getMachineSetting('extruder_amount')) > 1:
			res = res + " 1"
		res = res + " = " + getProfileSetting("print_temperature")
		if int(getMachineSetting('extruder_amount')) > 1:
			res = res + ", Nozzle 2 = " + getProfileSetting("print_temperature2")
		if getMachineSetting('has_heated_bed') == 'True':
			res = res + ", Bed = " + getProfileSetting("print_bed_temperature")
		return pre + res
	if tag == 'time':
