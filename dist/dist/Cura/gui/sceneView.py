...
class SceneView(openglGui.glGuiPanel):
	def __init__(self, parent):
		...
		self.orderButton = openglGui.glButton(self, 26, _("Shop"), (4,0), lambda value : webbrowser.open('http://3dk.in/'))
		# self.iSliceButton.setDisabled(True)

		self.printButton2         = openglGui.glButton(self, 6, _("USB Print"), (2,0), self.OnPrintButton)

		# self.complaintButton = openglGui.glBasicButton(self, _("Support"), (7,0), lambda value: webbrowser.open('https://desk.zoho.com/portal/fracktalworks/newticket'))
		self.supportButton = openglGui.glButton(self, 21, _("Support"), (5,0), lambda value: webbrowser.open('http://3dk.in/'))
	...
	def _get_obj_machine_nozzle_filename(self):
		return "%s_%s_%s" % (profile.getMachineSetting("machine_variant").upper(),
												 self._scene._objectList[0].getName().title().replace(" ", ""), 
												 str(profile.getProfileSettingFloat('nozzle_size'))
												)
		...
		if button == 1:
			...
			if len(removableStorage.getPossibleSDcardDrives()) > 0 and (connectionGroup is None or connectionGroup.getPriority() < 0):
				...
				if len(drives) > 1:
					...
					drive = drives[0]
				filename = self._get_obj_machine_nozzle_filename() + profile.getGCodeExtension()
	...
	def _saveGCode(self, targetFilename, ejectDrive = False):
		...
		try:
			...
			filename_temp = self._get_obj_machine_nozzle_filename() + '.stl'
			filename_temp_scad = self._get_obj_machine_nozzle_filename() + '.scad'
