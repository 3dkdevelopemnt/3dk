...
class FirstInfoPage(InfoPage):
	def __init__(self, parent, addNew):
		if addNew:
			...
			self.AddText(_("Welcome, and thanks for choosing 3DK!"))
			self.AddSeperator()
		self.AddText(_("This wizard will help you in setting up 3DK for your machine."))
		if not addNew:
			self.AddSeperator()
	...
	def StoreData(self):
		...
		for item in self._items:
			if item.GetValue():
				...
				if data[10]:
					profile.setAlterationFile('start.gcode', """;Sliced for {machine_name} - {nozzle_size} nozzle
...
class OtherMachineInfoPage(InfoPage):
	def __init__(self, parent):
		super(OtherMachineInfoPage, self).__init__(parent, _("3DK Ready!"))
		self.AddText(_("3DK is now ready to be used!"))
